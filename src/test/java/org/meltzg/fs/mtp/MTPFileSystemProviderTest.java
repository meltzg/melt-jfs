...
//    }
    ...
    public void validateUri() throws URISyntaxException, IOException {
        provider.validateURI(new URI(String.format("mtp://%s/", deviceIdentifier.toString())));
        provider.validateURI(new URI(String.format("mtp://%s", deviceIdentifier.toString())));
    }

    @Test(expected = IllegalArgumentException.class)
    public void validateUriNoSchema() throws URISyntaxException, IOException {
        var uri = new URI(String.format("/%s/", deviceIdentifier.toString()));
        provider.validateURI(uri);
    }

    @Test(expected = IllegalArgumentException.class)
    public void validateUriBadDeviceIdentifier() throws URISyntaxException, IOException {
        var uri = new URI(String.format("/%s/", "not:3:parts:aah"));
        provider.validateURI(uri);
    }

    @Test(expected = FileSystemNotFoundException.class)
    public void validateUriDeviceNotFound() throws URISyntaxException, IOException {
